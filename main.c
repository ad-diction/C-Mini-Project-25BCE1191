...
int main(void) {
    ...
    do {
        ...
        printf("7. Update Passenger Name\n");
        printf("0. Exit\n");
        printf("Enter choice: ");
        fflush(stdout);
...
/* ============================================================
   updateBooking
   Allows updating the passenger name on an active ticket.
   ============================================================ */
int updateBooking(void) {
    int ticketID;
    char buf[BUF];
    ...
    for (int i = 0; i < totalBookings; i++) {
        if (bookings[i].ticketID == ticketID) {
            if (bookings[i].isCancelled) {
                printf("[Error] Cannot update a cancelled ticket.\n");
                return -1;
            }
            printf("Current Passenger Name: %s\n", bookings[i].passengerName);
            ...
            strncpy(bookings[i].passengerName, buf, 49);
            bookings[i].passengerName[49] = '\0';
            saveBookingsToFile();
            printf("[Success] Passenger name updated to: %s\n",
                   bookings[i].passengerName);
            return 0;
        }
    }
